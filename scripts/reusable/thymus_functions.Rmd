...
get_sample_groups = function(analyses = "whole_organ", combine = F){
  ...
  if( identical( analyses, "whole_organ")) {
    ...
    return( merge_list_or_not( c(whole_organ ) ) )
  } else {
    to_return = c( whole_organ, RA_perturbation ) [analyses]
    if(length(to_return) == 0){ warning("No samples matched the analytic theme given.") }
    return( merge_list_or_not( to_return ) )
  }
}

cross_platform_data_loader = function (sample_ids = "all", test_mode = F, lincRNA = F) {
  ...
  print(paste("Reading from metadata a column named:", file_type_header))
  print("Result:")
  print(metadata[[ file_type_header ]])
  assertthat::are_equal(file_type_header, "dge_path")
  
  metadata[[ file_type_header ]] = file.path(dirname(get_metadata_path()), metadata[[ file_type_header ]])
